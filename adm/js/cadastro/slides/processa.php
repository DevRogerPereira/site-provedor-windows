<?php
	...
	if( $_SERVER['REQUEST_METHOD'] == 'POST' )
	{
		...
		if( !getPost('id') )
		{
			...
			//$query = mysqli_query($conexao,"UPDATE posts_carrinho SET cat_id = '".getPost('categorias')."' WHERE ses_id = '".getPost('ses_id')."'");
				...
				foreach($ativo as $valor){
					...
					$sql = "INSERT INTO slides VALUES (NULL, 'N', '".$valor."', '', '', '', '')";
				...
				if ($resultado) {
					...
					echo "<script language=\"javascript\">window.location=\"" . $urlsite . "/adm/slides-img/".$query_id."\"</script>";
		...
		else
		{
			...
			$query = mysqli_query($conexao,"UPDATE slides SET locl_id = '".getPost('locl_id')."', titulo = '".getPost('titulo')."' WHERE id = '".getPost('id')."'");

			if ($query) {

				echo "<script language=\"javascript\">window.location=\"" . $urlsite . "/adm/slides-img/".getPost('id')."\"</script>";
