...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    for ($i = 0; $i < count($_FILES['files']['name']); $i++) {
        ...
        $targetFilePath = $targetDir . uniqid() . '-inicial-' . str_replace(' ', '-', mb_strtolower($fileName));
        ...
        if (in_array($fileType, $allowTypes)) {
            ...
            if (move_uploaded_file($tempFilePath, $targetFilePath)) {
				...
				$stmt = $conn->prepare("UPDATE tb_config SET foto01 = ? WHERE id = 1");
				if ($stmt) {
					$stmt->bind_param("s", $dados_targetFilePath);
					$stmt->execute();
					$stmt->close();
				}
